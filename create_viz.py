...
class CreateViz_OT_Operator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # TODO create a seperate 3dview window
        ...
        def generate_empties(nodePack: nodeLib.structure.node_structs.NodePack_Struct):
            def corner_piece_generator(regenerate=True):
                ...
                def generate_obj():
                    bpy.ops.mesh.primitive_cube_add(
                        enter_editmode=True, location=(0, 0, 0))
            ...
            layer_coll = bpy.context.view_layer.layer_collection.children[
                'Node_data_Viz'].children[-1]
            bpy.context.view_layer.active_layer_collection = layer_coll
            ...
            def generate_rand_pos():
                ...
                Debug_Tools.debug_msg(
                    'available locations: {}'.format(available_locations), False)
            ...
            for count, node_ in enumerate(nodePack.pack[1:]):
                ...
                next_loc = bpy.data.objects['corner_piece'].data.vertices[rand_pos].co + \
                    curr_node_empty.location
                occupied_locs.append(next_loc)
                # TODO create the next node empty
                bpy.ops.object.empty_add(
                    type='PLAIN_AXES', radius=0.5, align='WORLD', location=next_loc, rotation=(0, 0, 0))
                bpy.context.active_object.name = 'node.000'

            # TODO delete corner_piece if not needed
        ...
        bpy.context.active_object.name = "viz"
        bpy.ops.object.text_add(radius=0.2, enter_editmode=False, align='WORLD', location=(
            0, 0, 0.2), rotation=(math.radians(90), 0, 0))
        bpy.context.active_object.name = "node_ID"
        bpy.data.objects['viz'].parent = bpy.data.objects['node.000']
        bpy.data.objects['node_ID'].parent = bpy.data.objects['node.000']
